...
export function useUserData() {
      ...
      if (subjectData) {
        const updatedSubjectProgress = {
          ...
          [subjectKey]: {
            ...
            progress: Math.round(
              ((subjectData.lessonsCompleted + 1) / subjectData.totalLessons) *
                100
            ),
        ...
        // Local update first - batched write happens automatically
        ...
        // Check and award badges (also batched)
        await checkAndAwardBadges(
          newTotalLessons,
          newLevel,
          updatedSubjectProgress
        );
      }
    } catch (error) {
  ...
  // Update streak (call on daily login) - Only writes if it's a new day
  const updateStreak = async () => {
    ...
    try {
      ...
      // Check if we already updated today - compare dates only (ignore time)
      if (lastLogin) {
        ...
        if (lastLoginDateOnly.getTime() === todayDateOnly.getTime()) {
          console.log("✅ Already logged in today, skipping streak update");
          return;
        }
      }
      ...
      // Only write to database if streak actually changed or it's first login
      console.log(
        "🔥 Updating streak:",
        newStreak,
        "(was:",
  ...
  const checkAndAwardBadges = async (
    ...
    Object.keys(subjectProgress).forEach((subject) => {
      ...
      if (
        progress >= 100 &&
        !userProfile.badges.includes(`${subject}_master`)
      ) {
    ...
    if (
      ...
      !userProfile.badges.includes("7_day_streak")
    ) {
      newBadges.push("7_day_streak");
    }
    if (
      userProfile.streak >= 30 &&
      !userProfile.badges.includes("30_day_streak")
    ) {
